...
class Renderer:
    def __init__(self, sh_degree=3, white_background=True, radius=1, use_surfels=True):

        self.sh_degree = sh_degree
        self.white_background = white_background
        self.radius = radius
        self.use_surfels = use_surfels
        if self.use_surfels:
            from scene.gaussian_model_2d import GaussianModel
        else:
    ...
    def extract_mesh(self, path, density_thresh=1, resolution=128, decimate_target=1e5):
        if self.use_surfels:
            ...
            vertices, triangles = self.gaussians.extract_mesh(
                path, density_thresh, resolution, decimate_target
            )
