...
def optimize_stage_1(
    image_block: Image.Image, preprocess_chk: bool, elevation_slider: float
):
    ...
    if preprocess_chk:
        ...
        subprocess.run(
            f'python process.py {os.path.join("tmp_data", "tmp.png")}', shell=True
        )
    else:
    ...
    # stage 1
    subprocess.run(
        f'python main.py --config {os.path.join("configs", "image.yaml")} input={os.path.join("tmp_data", "tmp_rgba.png")} save_path=tmp mesh_format=glb elevation={elevation_slider} force_cuda_rast=True',
        shell=True,
    )
...
def optimize_stage_2(elevation_slider: float):
    # stage 2
    subprocess.run(
        f'python main2.py --config {os.path.join("configs", "image.yaml")} input={os.path.join("tmp_data", "tmp_rgba.png")} save_path=tmp mesh_format=glb elevation={elevation_slider} force_cuda_rast=True',
        shell=True,
    )
...
if __name__ == "__main__":
    ...
    example_fns.sort()
    examples_full = [
        os.path.join(example_folder, x) for x in example_fns if x.endswith(".png")
    ]
        ...
        with gr.Row(variant="panel"):
            with gr.Column(scale=5):
                ...
                elevation_slider = gr.Slider(
                    -90, 90, value=0, step=1, label="Estimated elevation angle"
                )
                gr.Markdown(
            ...
            with gr.Column(scale=5):
                obj3d_stage1 = gr.Model3D(
                    clear_color=[0.0, 0.0, 0.0, 0.0], label="3D Model (Stage 1)"
                )
                obj3d = gr.Model3D(
                    clear_color=[0.0, 0.0, 0.0, 0.0], label="3D Model (Final)"
                )

            # if there is an input image, continue with inference
            # else display an error message
            img_run_btn.click(
                check_img_input, inputs=[image_block], queue=False
            ).success(
                optimize_stage_1,
                inputs=[image_block, preprocess_chk, elevation_slider],
                outputs=[obj3d_stage1],
            ).success(
